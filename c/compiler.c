...
static void importStatement(Parser *parser) {
    if (check(parser, TOKEN_LEFT_PAREN)) {
        consume(parser, TOKEN_LEFT_PAREN, "Expect '(' after import.");
        expression(parser);
        consume(parser, TOKEN_RIGHT_PAREN, "Expect ')' after import expression.");
    } else {
        consume(parser, TOKEN_STRING, "Expect string after import.");
        emitConstant(parser, OBJ_VAL(copyString(parser->vm, parser->previous.start + 1,
