...
void setcurrentFile(VM *vm, const char *scriptname) {
    push(vm, OBJ_VAL(copyString(vm, scriptname, (int) strlen(scriptname))));
    tableSet(vm, &vm->globals, copyString(vm, "__FILE__", 8), OBJ_VAL(copyString (vm, vm->currentScriptName, strlen (vm->currentScriptName))));
    pop(vm);
}
...
static InterpretResult run(VM *vm) {
    ...
    INTERPRET_LOOP
    {
        ...
        CASE_CODE(IMPORT): {
            ...
            setcurrentFile(vm, vm->currentScriptName);
